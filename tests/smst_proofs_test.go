...
func TestSMST_Proof_Operations(t *testing.T) {
	...
	result, err = smt.VerifySumProof(proof, smt.Placeholder(base), []byte("testKey3"), smt.DefaultValue, 0, base)
	require.NoError(t, err)
	require.True(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey3"), []byte("badValue"), 5, base)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	checkCompactEquivalence(t, proof, base)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey"), []byte("testValue"), 5, base) // valid
	require.NoError(t, err)
	require.True(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey"), []byte("badValue"), 5, base) // wrong value
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey"), []byte("testValue"), 10, base) // wrong sum
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey"), []byte("badValue"), 10, base) // wrong value and sum
	require.NoError(t, err)
	require.False(t, result)
	...
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(randomiseSumProof(proof), root, []byte("testKey"), []byte("testValue"), 5, base) // invalid proof
	require.NoError(t, err)
	require.False(t, result)
	...
	require.NoError(t, err)
	checkCompactEquivalence(t, proof, base)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey2"), []byte("testValue"), 5, base) // valid
	require.NoError(t, err)
	require.True(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey2"), []byte("badValue"), 5, base) // wrong value
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey2"), []byte("testValue"), 10, base) // wrong sum
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey2"), []byte("badValue"), 10, base) // wrong value and sum
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(randomiseSumProof(proof), root, []byte("testKey2"), []byte("testValue"), 5, base) // invalid proof
	require.NoError(t, err)
	require.False(t, result)

	// Try proving a default value for a non-default leaf.
	var sum [smt.SumSize]byte
	binary.BigEndian.PutUint64(sum[:], 5)
	tval := base.DigestValue([]byte("testValue"))
	tval = append(tval, sum[:]...)
	_, leafData := base.TreeHasher().DigestSumLeaf(base.PathHasher().Path([]byte("testKey2")), tval)
	proof = &smt.SparseMerkleProof{
		SideNodes:             proof.SideNodes,
		NonMembershipLeafData: leafData,
	}
	result, err = smt.VerifySumProof(proof, root, []byte("testKey2"), smt.DefaultValue, 0, base)
	...
	require.NoError(t, err)
	checkCompactEquivalence(t, proof, base)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey3"), smt.DefaultValue, 0, base) // valid
	require.NoError(t, err)
	require.True(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey3"), []byte("badValue"), 0, base) // wrong value
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(proof, root, []byte("testKey3"), smt.DefaultValue, 5, base) // wrong sum
	require.NoError(t, err)
	require.False(t, result)
	result, err = smt.VerifySumProof(randomiseSumProof(proof), root, []byte("testKey3"), smt.DefaultValue, 0, base) // invalid proof
	require.NoError(t, err)
	require.False(t, result)
...
func TestSMST_Proof_ValidateBasic(t *testing.T) {
	...
	result, err := smt.VerifySumProof(proof, root, []byte("testKey1"), []byte("testValue1"), 1, base)
	require.ErrorIs(t, err, smt.ErrBadProof)
	require.False(t, result)
...
func TestSMST_ClosestProof_ValidateBasic(t *testing.T) {
	smn, err := badger.NewKVStore("")
	require.NoError(t, err)
	smst := smt.NewSparseMerkleSumTree(smn, sha256.New())
	np := smt.NoPrehashSpec(sha256.New(), true)
	base := smst.Spec()
...
func TestSMST_ProveClosest(t *testing.T) {
	...
	smn, err = badger.NewKVStore("")
	require.NoError(t, err)
	smst = smt.NewSparseMerkleSumTree(smn, sha256.New(), smt.WithValueHasher(nil))
	...
	result, err = smt.VerifyClosestProof(proof, root, smt.NoPrehashSpec(sha256.New(), true))
	require.NoError(t, err)
	require.True(t, result)
...
func TestSMST_ProveClosest_Empty(t *testing.T) {
	...
	require.Equal(t, proof, &smt.SparseMerkleClosestProof{
		...
		FlippedBits:  []int{0},
		Depth:        0,
		ClosestPath:  smt.Placeholder(smst.Spec()),
		ClosestProof: &smt.SparseMerkleProof{},
	})

	result, err := smt.VerifyClosestProof(proof, smst.Root(), smt.NoPrehashSpec(sha256.New(), true))
	require.NoError(t, err)
	require.True(t, result)
...
func TestSMST_ProveClosest_Proof(t *testing.T) {
	var smn smt.KVStore
	var smst256 *smt.SMST
	var smst512 *smt.SMST
	...
	smn, err = badger.NewKVStore("")
	require.NoError(t, err)
	smst256 = smt.NewSparseMerkleSumTree(smn, sha256.New())
	smst512 = smt.NewSparseMerkleSumTree(smn, sha512.New())
