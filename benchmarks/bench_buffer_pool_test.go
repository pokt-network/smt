...
// BenchmarkNode_CreationPatterns tests CPU and memory usage for operations that create different node types
func BenchmarkNode_CreationPatterns(b *testing.B) {
	scenarios := []struct {
		name      string
		valueSize int
	}{
...
// BenchmarkBufferPool_MemoryPressure tests memory allocation patterns under different buffer pool usage
func BenchmarkBufferPool_MemoryPressure(b *testing.B) {
	scenarios := []struct {
		name      string
		numOps    int
		valueSize int
...
// BenchmarkTrie_ResourceUsage tests overall trie operations with resource monitoring
func BenchmarkTrie_ResourceUsage(b *testing.B) {
	nodes := simplemap.NewSimpleMap()
	trie := smt.NewSparseMerkleTrie(nodes, sha256.New())
...
// BenchmarkBufferPool_Contention tests buffer pool performance under concurrent access
func BenchmarkBufferPool_Contention(b *testing.B) {
